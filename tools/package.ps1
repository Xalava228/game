...
foreach($required in @('index.html','sdk-bridge.js','platform-config.js','icon.png','Build')) {
...
$zip=Join-Path $release 'TimeThief-Yandex-1.7.0.zip'
Compress-Archive -Path (Join-Path $build '*') -DestinationPath $zip -Force
$hash=Get-FileHash -LiteralPath $zip -Algorithm SHA256
($hash.Hash+'  TimeThief-Yandex-1.7.0.zip') | Set-Content -LiteralPath (Join-Path $release 'SHA256.txt')
