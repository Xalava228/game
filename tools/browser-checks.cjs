...
(async()=>{
 ...
 try {
  ...
  await click(1050,809,30);for(let i=0;i<6;i++)await click(800,470,140);await page.waitForTimeout(700);s=await save();check(s.phase==='Victory'&&s.defeated===1,'taps defeat the magic keeper');await snap('03-victory');
  ...
  await click(1050,809,220);await click(1504,48,30);const paused=await save();await page.waitForTimeout(600);check(Math.abs((await save()).player.CurrentTime-paused.player.CurrentTime)<.001,'manual pause freezes time');await snap('06-pause');await click(800,576,25);
