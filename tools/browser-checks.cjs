...
(async()=>{
 ...
 const click=async(x,y,delay=220)=>{await page.mouse.click(x,y);await page.waitForTimeout(delay);};
 ...
 try {
  ...
  await click(448,639);await snap('02-intro');let s=await save();check(s.level===1&&s.phase==='Intro','new run opens paused tutorial');
  await click(1026,716,30);await page.mouse.move(800,470);await page.mouse.down();await page.waitForTimeout(2100);await page.mouse.up();await page.waitForTimeout(700);s=await save();check(s.phase==='Victory'&&s.defeated===1,'hold defeats first keeper');await snap('03-victory');
  ...
  await click(1026,716,220);await click(1504,48,30);const paused=await save();await page.waitForTimeout(600);check(Math.abs((await save()).player.CurrentTime-paused.player.CurrentTime)<.001,'manual pause freezes time');await snap('06-pause');await click(800,576,25);
  await page.mouse.move(800,470);await page.mouse.down();await page.waitForTimeout(2300);await page.mouse.up();await page.waitForTimeout(700);s=await save();check(s.phase==='Victory'&&s.defeated===2,'resume and magic work');
  ...
  await page.reload();await ready();await click(448,639);s=await save();check(s.level===2&&s.phase==='Victory'&&s.shop.maxTimePurchases===1,'reload restores run and shop state');
