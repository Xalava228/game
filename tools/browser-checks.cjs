...
(async()=>{
 ...
 try {
  ...
  await click(800,842,30);for(let i=0;i<6;i++)await click(800,470,140);await page.waitForTimeout(700);s=await save();check(s.phase==='Victory'&&s.defeated===1,'taps defeat the magic keeper');await snap('03-victory');
  ...
  await click(904,779);await snap('04-stats');await click(1200,824);await click(1304,779);await snap('05-shop');
  await click(1200,860);await click(1104,851);s=await save();check(s.level===2&&s.phase==='Intro','continue advances exactly one level');
  await click(800,842,220);await click(1411,48,30);const paused=await save();await page.waitForTimeout(600);check(Math.abs((await save()).player.CurrentTime-paused.player.CurrentTime)<.001,'manual pause freezes time');await snap('06-pause');await click(800,576,25);
  ...
  await click(1304,779);const before=await save();await click(280,485);const purchased=await save();check(Math.abs(purchased.player.MaxTime-before.player.MaxTime-1)<.0001&&purchased.player.CurrentTime<before.player.CurrentTime,'shop spends life and increases capacity without healing');
  ...
  const fixture={...s,level:25,phase:'Intro',bossPhase2:true,enemyTime:.8,enemyTimer:4,elapsed:0,enemyAttacks:0,physicalHits:0,rewardDoubled:false,player:{...s.player,CurrentTime:30,MaxTime:30,Attack:10}};
  await page.evaluate(f=>window.name='TT_FIXTURE:'+JSON.stringify({...f,updatedAt:Date.now()}),fixture);await page.reload();await ready();await click(424,587);await snap('07-boss-intro');await click(800,842,220);await click(646,446,200);await page.waitForTimeout(750);s=await save();check(s.phase==='RewardSelection','boss defeat opens three rewards');await snap('08-rewards');await click(304,629,750);s=await save();check(s.phase==='Victory'&&s.totalBossesDefeated===1&&s.lastGift,'exactly one boss reward is committed and described');
  await page.setViewportSize({width:390,height:844});await page.waitForTimeout(450);await snap('09-mobile-results');await click(103,684);await snap('10-mobile-stats');await click(292,773,800);await click(287,684);await snap('11-mobile-shop');const mobileBefore=await save();await click(304,315);check((await save()).shop.maxTimePurchases===mobileBefore.shop.maxTimePurchases+1,'portrait shop button buys an upgrade');
