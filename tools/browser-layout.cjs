...
await page.evaluate(f=>window.name='TT_FIXTURE:'+JSON.stringify(f),fixture);await page.setViewportSize({width:390,height:844});await page.reload();await ready();await click(.5,.777);await page.screenshot({path:__dirname+'/../QA/21-last-boss-mobile.png'});
