...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(f=>window.name='TT_FIXTURE:'+JSON.stringify(f),fixture);await page.reload();await ready();await click(.265,.652);await click(.654,.89);await click(.5,.776);
  ...
  const b=await page.locator('canvas').boundingBox();await page.mouse.move(b.x+b.width*.5,b.y+b.height*.47);await page.mouse.down();await page.waitForTimeout(2300);await page.mouse.up();after=await save();
  // Opening tactics consumed no combat time; sufficient sustained counter damage crosses half reserve.
  if(!after.bossPhase2){await page.mouse.down();await page.waitForTimeout(2300);await page.mouse.up();after=await save();}
