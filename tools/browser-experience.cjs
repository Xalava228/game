...
(async()=>{
 ...
 try {
   ...
   const player={...base.player,CurrentTime:30,MaxTime:30,Attack:.5,CritChance:.75};
   await fixture({level:125,phase:'Intro',enemyTime:10,enemyTimer:.35,enemyAttacks:2,elapsed:0,physicalHits:0,player});
   await click(1026,716,900);await snap('13-heavy-boss-strike');
   let s=await save();check(s.enemyAttacks===3&&s.player.CurrentTime<29,'third boss strike removes time');
   await click(1504,48);const paused=await save();await page.waitForTimeout(500);check(Math.abs((await save()).player.CurrentTime-paused.player.CurrentTime)<.001,'pause stops combat after heavy strike');check(await energy()<.00001,'pause silences music');
   ...
   await fixture({level:3,phase:'Intro',enemyTime:4,enemyTimer:4,enemyAttacks:0,elapsed:0,physicalHits:0,player});
   await click(1026,716);await click(800,430,120);await snap('14-player-hit');s=await save();check(s.physicalHits===1&&s.enemyTime<4,'tap creates exactly one physical attack');
   const physical=s.physicalHits;await page.mouse.move(800,430);await page.mouse.down();await page.waitForTimeout(550);await page.mouse.move(40,40);await page.waitForTimeout(60);const cancelled=await save();await page.waitForTimeout(250);await page.mouse.up();s=await save();check(s.physicalHits===physical&&Math.abs(s.enemyTime-cancelled.enemyTime)<.001,'leaving target cancels magic without a release hit');
   ...
   await click(1026,716,1500);s=await save();check(s.phase==='GameOver'&&s.player.CurrentTime===0,'time expiry opens game over');await snap('15-game-over');
