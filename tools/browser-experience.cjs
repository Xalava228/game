...
(async()=>{
 ...
 try {
   ...
   await click(1046,801);await click(800,430,120);await snap('14-player-hit');s=await save();check(s.physicalHits===1&&s.enemyTime<4+s.elapsed-.4,'tap creates exactly one physical attack while passive time still transfers');
   ...
   await click(1046,801,1500);s=await save();check(s.phase==='GameOver'&&s.player.CurrentTime===0,'time expiry opens game over');await snap('15-game-over');
