...
try{
 ...
 await tap(286,758,80);for(let i=0;i<6;i++)await tap(195,430,130);await page.waitForTimeout(750);let s=await save();check(s.phase==='Victory'&&s.defeated===1&&s.physicalHits>=3,'touch taps defeat magic keeper');
 await tap(287,767);check((await save()).level===2,'touch advances one level');await tap(286,758);await tap(195,430,150);s=await save();check(s.physicalHits===1,'short touch is one physical attack');
