...
try{
 ...
 await tap(195,498);check((await save()).phase==='Intro','touch starts a run');await snap('18-mobile-intro');
 await tap(283,672,80);await hold(2200);let s=await save();check(s.phase==='Victory'&&s.defeated===1&&s.physicalHits===0,'touch hold wins without an extra tap');
 await tap(287,767);check((await save()).level===2,'touch advances one level');await tap(283,672);await tap(195,430,150);s=await save();check(s.physicalHits===1&&s.enemyTime<4,'short touch is one physical attack');
 await hold(1900);check((await save()).phase==='Victory','tap and hold can be combined on mobile');
