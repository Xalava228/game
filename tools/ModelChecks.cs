...
class ModelChecks {
 ...
 static void Main(){var c=Config();
  ...
  foreach(var ability in new[]{Ability.None,Ability.HeavyStrike,Ability.MagicSurge,Ability.Leech}){
   game=new GameManager{config=c};game.player.MaxTime=game.player.CurrentTime=1000;game.player.Armor=7;game.player.MagicResistance=19;
   enemy=new EnemyController(game,EnemyGenerator.Generate(c,125,1));enemy.data.ability=ability;enemy.attackCount=2;enemy.bossPhase2=true;enemy.timer=0;
